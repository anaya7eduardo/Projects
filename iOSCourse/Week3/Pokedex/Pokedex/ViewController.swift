...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        /*
         ...
         case weight = "weight"
         */
        pokemonDetails = Array(repeating: PokemonSelected(abilities: [Abilities(ability: Ability(name: "H1")),Abilities(ability: Ability(name: "H2"))], height: -1, id: -1, moves: [Moves(move: Move(name: "M1")),Moves(move: Move(name: "M2"))], name: "N1", sprites: nil, types: [Types(typ: Typ(name: "T1"))], weight: -1), count: 150)
        fetchFirstData()
        setupDelegates()
        super.viewDidLoad()
...
extension ViewController: UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        ...
        if let cell = cell as? PokemonTableViewCell {
            cell.pokemonSprintView.getImage(from: (URL(string: spriteURL+String(indexPath.row+1)+".png"))!)
            //newCell.pokemonSprintView.image = UIImage(named: "\(pokemon[indexPath.row].url)")
            cell.setupCell(pokemon: pokemon[indexPath.row], pokemonNumber: "#\(indexPath.row+1)")
        }
        return cell
        */
    }
    ...
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        ...
        /*
        print("\(pokemon[indexPath.row].url) Selected")
        if let vc = storyboard?.instantiateViewController(withIdentifier: "DetailViewController") as? DetailViewController {
            vc.pokemonName = "\(pokemon[indexPath.row].name.capitalized)"
            vc.pokemonNumber = "\(indexPath.row+1)"
            self.navigationController?.pushViewController(vc, animated: true)
        }
        tableView.deselectRow(at: indexPath, animated: true)
        */
